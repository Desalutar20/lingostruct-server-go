...
func (s *Service) ResetPassword(ctx context.Context, data *dto.ResetPasswordRequest) error {
	key := s.generateResetPasswordkey(data.Token)
	...
	if user == nil || user.Email != data.Email || user.IsBanned || !user.IsVerified {
		return apperror.New(apperror.Conflict, "invalid token")
	}
